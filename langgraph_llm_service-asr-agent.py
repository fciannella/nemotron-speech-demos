...
class LangGraphLLMService(OpenAILLMService):
    ...
    async def _stream_langgraph(self, text: str) -> None:
        ...
        # LangGraph expects input as a dict with 'messages' key, not a raw array
        input_dict = {"messages": input_messages}
        
        # Use 'messages' stream mode for token-by-token streaming
        # This gives us messages/partial events with incremental content
        stream_modes = ["messages"] if self.stream_mode == "messages" else [self.stream_mode]
        
        try:
            logger.info(f"[LangGraph] Starting SDK stream with modes: {stream_modes}...")
            async for chunk in self._client.runs.stream(
                self._thread_id,
                self.assistant,
                input=input_dict,  # Send as dict with 'messages' key
                        ...
                        if isinstance(msgs, list) and msgs:
                            ...
                            if isinstance(last_msg, dict):
                                ...
                                if role in ("assistant", "ai") and isinstance(content, str) and content.strip():
                                    ...
                                    if not self._outer_open and content != self._last_content:
                                        self._last_content = content
                                        await self.push_frame(LLMFullResponseStartFrame())
                                        # In values mode, emit full content at once
                                        await self.push_frame(LLMTextFrame(_tts_sanitize(content)))
                                        await self.push_frame(LLMFullResponseEndFrame())
                                        logger.info(f"[LangGraph] Emitted final response (values mode, {len(content)} chars)")
